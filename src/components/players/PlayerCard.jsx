...
export default function PlayerCard({ player, onClick }) {
  ...
  useEffect(() => {
    // If Firestore has a photo use it
    ...
    fetch(`/api/sportsdb?path=${encodeURIComponent(`/searchplayers.php?p=${encodeURIComponent(player.name)}`)}`)
      .then(r => r.ok ? r.json() : null)
      .then(json => {
        const p = json?.player?.[0];
        const url = p?.strThumb || p?.strCutout || p?.strRender || '';
        if (url) { photoCache[player.name] = url; setPhoto(url); }
      })
      .catch(() => {});
